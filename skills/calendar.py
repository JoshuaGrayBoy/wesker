...
class Calender_skill():
    ...
    def add_event(self, wesker:AI)->bool:
        wesker.say("What is the name of the event")
        try:
            event_name = wesker.listen()
            wesker.say("When is this?")
            event_begin = wesker.listen()
            event_isodate = dateparser.parse(event_begin).strftime("%Y-%m-%d %H:%M:%S")
            wesker.say("What are you doing then?")
            event_description = wesker.listen()
            message = "Ok, I'll remind you of " + event_name + "when it's pertinent"
            wesker.say(message)
            ...
            return False
    def remove_event(self, wesker:AI)->bool:
        wesker.say("Who are we ditching now, dear?")
        try:
            event_name = wesker.listen()
            try:
                self.calendar.remove_event(event_name=event_name)
                wesker.say("I won't remind you then")
                self.calendar.save()
                return True
            except:
                wesker.say("You never told me about",event_name)
                return False
        except:
            print("Wait, I'm confused")
            return False

    def list_events(self, period, wesker:AI)->bool:
